...
<?php
    ...
    {   
        ...
        $insert="insert into rentdet(pid,name,year,dept,pname) values('$id','$na','$yr','$dp','$pn')";
        $insert_q=mysqli_query($conn,$insert);
        if($insert_q)
        {
            $status="No";
            $update="update rent set Available='$status' where pid='$id'";
            $update_q=mysqli_query($conn,$update);
?>
            <div class="mess">
                <div class="alert" style="background-color: lime;">
                        <span class="closebtn" onclick="this.parentElement.style.display='none';">&times;</span> 
                        <strong>Product Rented!</strong> 
                </div>
            </div>
 <?php          
        }
