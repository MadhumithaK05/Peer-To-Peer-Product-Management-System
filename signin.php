...
<?php
    ...
    $logpwd=$_POST["pass"];
    if(empty($logemail) || empty($logpwd)){
        ?>
                   <div class="mess">
    ...
    $sql= "select Password, Name, Email_Id,Roll_No from users where Email_Id='$logemail'";
    $res=mysqli_query($conn, $sql);
    $acc=mysqli_fetch_array($res);

    if($logpwd == $acc["Password"] && $logpwd!=null){
        $_SESSION["Username"]=$acc["Name"];
        $_SESSION["Email"]=$acc["Email_Id"];
        $_SESSION["ID"]=$acc["Roll_No"];
